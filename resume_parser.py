...
with MongoDB Storage Integration
"""
...
try:
    ...
    resumes_collection = db[MONGODB_COLLECTION_NAME]
    
    # Create indexes for better query performance
    resumes_collection.create_index([("parsed_data.name", "text")])
    resumes_collection.create_index([("parsed_data.email", 1)])
    resumes_collection.create_index([("parsed_data.skills", 1)])
    resumes_collection.create_index([("metadata.upload_date", -1)])
...
class ResumeParser:
    ...
    def store_resume_in_db(self, file_path: str, parsed_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            document = {
                'metadata': {
                    ...
                    'processing_time': datetime.datetime.utcnow()
                },
                'extracted_text': self.extract_text_from_file(file_path)[:10000],  # Store first 10k chars
                'parsed_data': parsed_data,
                'embeddings': {
                    'text_embedding': resume_embeddings[-1].tolist() if resume_embeddings else None,
            ...
            logger.info(f"Stored resume in MongoDB with ID: {result.inserted_id}")
            return True
            
        except Exception as e:
            logger.error(f"Error storing resume in MongoDB: {e}")
            return False

    def search_resumes_in_db(self, query: Dict[str, Any], limit: int = 10) -> List[Dict[str, Any]]:
        """Search resumes in MongoDB based on query"""
        if not mongo_client:
            logger.warning("MongoDB not available - cannot search")
            return []
        
        try:
            results = list(resumes_collection.find(query).limit(limit))
            ...
            logger.error(f"Error searching resumes in MongoDB: {e}")
            return []

    def text_search_resumes(self, search_text: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Perform full-text search on resumes"""
        if not mongo_client:
            logger.warning("MongoDB not available - cannot search")
            return []
        ...
        try:
            ...
            if "parsed_data.name_text" not in resumes_collection.index_information():
                resumes_collection.create_index([("parsed_data.name", "text"), 
    ...
    def check_for_duplicate(self, email: Optional[str], phone: Optional[str]) -> bool:
        """Check if a resume with the same email or phone already exists"""
        if not mongo_client:
            return False
        
        query = {"$or": []}
        if email:
            query["$or"].append({"parsed_data.email": email})
        if phone:
            query["$or"].append({"parsed_data.phone": phone})
        
        if not query["$or"]:
            return False
        
        return resumes_collection.count_documents(query) > 0
    ...
    def parse_resume(self, file_path: str) -> Dict[str, Any]:
        ...
        # Store in MongoDB
        self.store_resume_in_db(file_path, result)
...
def search_resumes():
    ...
    try:
        ...
        results = parser.search_resumes_in_db(query, limit)
        return jsonify({'results': results})
...
@app.route('/api/resumes/text-search', methods=['POST'])
def text_search_resumes():
    ...
    try:
        ...
        results = parser.text_search_resumes(search_text, limit)
        return jsonify({'results': results})
...
def get_resume_stats():
    ...
    try:
        ...
        stats = {
            ...
            'latest_resumes': list(resumes_collection.find(
                {},
                {'metadata.filename': 1, 'metadata.upload_date': 1, '_id': 0}
            ).sort('metadata.upload_date', -1).limit(5)),
            'fields_availability': {
                'name': resumes_collection.count_documents({'parsed_data.name': {'$ne': None}}),
                'email': resumes_collection.count_documents({'parsed_data.email': {'$ne': None}}),
                'phone': resumes_collection.count_documents({'parsed_data.phone': {'$ne': None}}),
                'education': resumes_collection.count_documents({'parsed_data.education': {'$ne': None}})
            }
        }
...
def health_check():
    ...
    return jsonify({
        ...
        'stored_resumes': resumes_collection.count_documents({}) if mongo_status else 0,
        'vector_db_size': len(resume_texts)
    })
...
if __name__ == '__main__':
    ...
    ✅ Advanced querying of stored resumes
    ✅ Statistics and analytics endpoints
    ✅ Better error handling and logging
    
    MongoDB Setup:
    1. Install MongoDB Community Edition
    2. Make sure it's running (sudo systemctl start mongod on Linux)
    3. The system will automatically create needed indexes
    ...
    - /api/resumes/search : Search resumes in MongoDB
    - /api/resumes/text-search : Full-text search across resumes
    - /api/resumes/stats : Get statistics about stored resumes
    - /api/health : Health check
    
    Requirements Installation:
    pip install flask flask-cors PyMuPDF python-docx pandas sentence-transformers
    pip install scikit-learn faiss-cpu openpyxl numpy pymongo
